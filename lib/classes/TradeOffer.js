        ...
        switch (key) {
            case 'cancelTime':
                ...
                if (
                    ...
                    throw new Error(
                        `Cannot set cancelTime for offer #${this.id} as it is not active (${
                            ETradeOfferState[this.state]
                        }).`
                    );
                }
...
function addItem(details, offer, list) {
    ...
    if (
        ...
        (typeof details.assetid === 'undefined' && typeof details.id === 'undefined')
    ) {
...
TradeOffer.prototype.getExchangeDetails = function (getDetailsIfFailed, callback) {
    ...
    offer.manager._apiCall('GET', 'GetTradeStatus', 1, { tradeid: offer.tradeID }, (err, result) => {
        ...
        if (
            !getDetailsIfFailed &&
            [ETradeStatus.Complete, ETradeStatus.InEscrow, ETradeStatus.EscrowRollback].indexOf(trade.status) == -1
        ) {
...
TradeOffer.prototype.getUserDetails = function (callback) {
    ...
    this.manager._community.httpRequestGet(url, (err, response, body) => {
        ...
        // Avatars
        var myAvatar = body.match(
            new RegExp('<img src="([^"]+)"( alt="[^"]*")? data-miniprofile="' + this.manager.steamID.accountid + '">')
        );
        var theirAvatar = body.match(
            new RegExp('<img src="([^"]+)"( alt="[^"]*")? data-miniprofile="' + this.partner.accountid + '">')
        );
        if (myAvatar) {
