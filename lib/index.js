	...
	if (this._dataGzip) {
		Zlib.gzip(content, (err, data) => {
			if (err) {
				...
				this.storage.writeFile(filename + '.gz', data).catch(err => {
					this.emit('debug', `Cannot write ${filename}.gz: ${err.message}`);
				});
			}
		});
	} else {
		this.storage.writeFile(filename, content).catch(err => {
			if (err) {
...
TradeOfferManager.prototype._getFromDisk = function(filenames, callback) {
	...
	this.storage.readFiles(filenames).then(results => {
		var files = {};
		...
		if (this._dataGzip) {
			Async.mapValues(files, (content, filename, mapCb) => {
...
TradeOfferManager.prototype.createOffer = function(partner, token) {
	if (typeof partner === 'string' && partner.match(/^https?:\/\//)) {
		...
		partner = SteamID.fromIndividualAccountID(url.searchParams.get(partner));
...
TradeOfferManager.prototype.getOffers = function(filter, historicalCutoff, callback) {
	...
	var request = () => {
		this._apiCall('GET', 'GetTradeOffers', 1, options, (err, body) => {
			...
			if (typeof options.cursor == 'number' && options.cursor != 0 && options.cursor <= 100) {
